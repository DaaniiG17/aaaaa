...
try {
    ...
    if ($equipo) {
        ...
        $sql = "SELECT nombre, posicion, goles, imagen FROM $equipo";
        ...
        if (count($result) > 0) {
            ...
            foreach($result as $row) {
                ...
                echo '<br>';
                echo '<h5>' . $row["posicion"] . '<br>  Goles: ' . $row["goles"] . '</h5>';
                echo '<button class="btn btn-secondary bg-primary cambiarNumeroGoles">Cambiar número de goles</button>';
                echo '</div><!-- /.col-lg-4 -->';
            }
